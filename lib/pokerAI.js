...
const PERSONALITIES = {
  ...
  // Additional Casino Royale characters
  'Mr. White': {
    name: 'Mr. White',
    tightness: 0.92,      // Folds almost everything
    aggression: 0.95,     // When he plays, he bets huge
    bluffFrequency: 0.03, // Almost never bluffs - when he bets, he has it
    description: 'Almost invisible. Folds constantly, then strikes without warning.',
  },
  'Steven Obanno': {
    name: 'Steven Obanno',
    tightness: 0.22,      // Plays far too many hands
    aggression: 0.92,     // Hyper-aggressive, constantly betting
    bluffFrequency: 0.45, // Bluffs constantly - more intimidation than strategy
    tiltFactor: true,     // Gets more aggressive when losing
    description: 'Intimidating and impatient. Not here to play cards.',
  },
  'Alex Dimitrios': {
    name: 'Alex Dimitrios',
    tightness: 0.40,      // Loose, plays too many hands
    aggression: 0.50,     // Passive-aggressive, calls too much
    bluffFrequency: 0.30, // Bluffs at wrong times
    description: 'Nervous, out of his depth. Calls too much.',
  },
  'Valenka': {
    name: 'Valenka',
    tightness: 0.75,      // Tight, cautious
    aggression: 0.35,     // Passive
    bluffFrequency: 0.10, // Occasional bluffs
    description: 'Quiet and observant. Easy to underestimate.',
  },
  'Dryden': {
    name: 'Dryden',
    tightness: 0.60,      // Moderate
    aggression: 0.55,     // Moderate
    bluffFrequency: 0.25, // Overconfident bluffer
    description: 'Arrogant and condescending. Overconfident bluffer.',
  },
  'Mollaka': {
    name: 'Mollaka',
    tightness: 0.55,      // Moderate
    aggression: 0.60,     // Pushes when he senses weakness
    bluffFrequency: 0.20, // Occasional calculated bluffs
    description: 'Professional. Terse. Watches exits.',
  },
};
...
function makeAIDecision(player, gameState, personality) {
  ...
  if (shouldBluff && effectiveStrength < callThreshold && callAmount < pot * 0.5) {
    if (availableActions.includes('raise') && player.chips > callAmount + minRaise * 2) {
      ...
      if (actualRaise >= minRaise) {
        return { action: 'raise', amount: actualRaise, isBluff: true };
      }
    }
  }
  ...
  if (effectiveStrength >= raiseThreshold && availableActions.includes('raise')) {
    ...
    if (actualRaise >= minRaise && player.chips >= callAmount + actualRaise) {
      return { action: 'raise', amount: actualRaise, isBluff: false };
    }
  }
  ...
  if (availableActions.includes('check')) {
    ...
    if (effectiveStrength >= raiseThreshold * 0.9 && availableActions.includes('raise')) {
      ...
      if (actualBet >= minRaise) {
        return { action: 'raise', amount: actualBet, isBluff: false };
      }
    }
    return { action: 'check' };
  }
